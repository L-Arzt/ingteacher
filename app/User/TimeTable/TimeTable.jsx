        ...
        for (let i = 1; i < 7; i++) {
            let tablePart = (
                <TableRow className='transition-all duration-300 ease-in-out hover:bg-gray-100 shadow-sm rounded-md' key={`row-${i}`}>
                    <TableCell key={`time-${i}`} className="py-3 px-5 font-medium text-gray-700">
                        {TimeLessonS[i]}<hr />{TimeLessonPo[i]}
                    </TableCell>
                    {daysOfWeek.map((dayName, dayIndex) => {
                        ...
                        if (lesson) {
                            if (lesson.userId === currentUser?.id) {
                                // Current user is the one who booked this slot
                                return (
                                    <TableCell
                                        className="relative py-3 px-5 border-t border-gray-300"
                                ...
                                // Other user has booked the slot
                                return (
                                    <TableCell className="py-3 px-5 border-t border-gray-300" key={`occupied-${i}-${dayIndex}`}>
                                        <div className='flex w-[140px] h-[70px] items-center justify-center flex-col gap-2'>
                                            <h1 className='text-[#7E7E7E] font-medium'>Занято</h1>
                                        </div>
                                    </TableCell>
                                );
                            }
                        } else {
                            // No lesson booked in this slot
                            return (
                                <TableCell className="py-3 px-5 border-t border-gray-300" key={`empty-${i}-${dayIndex}`}>
                                    <div className='flex w-[140px] h-[70px] items-center justify-center flex-col gap-2'>
                                        <h1 className='text-[#7E7E7E] font-medium'>Свободно</h1>
                                        <Link
                                        ...
                                        >
                                            <button className="bg-[#FF9100] hover:scale-105 active:scale-100 transition-all duration-200 text-white font-semibold py-2 px-4 rounded-md shadow-md">
                                                Забронировать
                                            </button>
                                        </Link>
                                    </div>
